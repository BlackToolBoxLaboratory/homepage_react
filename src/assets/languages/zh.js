export default {
  ...
  application: {
    ...
    'fb-sdk': {
      name: '臉書開發套件',
      description:
        '此套件的核心並非原創套件，主要是以整理臉書（Facebook）提供的軟體開發套件，方便需要結合使用臉書服務的網站開發為目的，其中所有的參數控制皆可直接參照原文件只是控制，本站亦會提供整合所有功能的參數說明文件，提供使用者查詢參數方便。由於並非原創套件，主板號的編排依照使用的軟體開發套件版號，因此還請使用者注意參考文件與本套件的版本差異。',
    },
    languages: {
      name: '多語言切換器',
      description:
      ...
      trigger: {
        ...
        dispatch:
          '我們可以結合儲存管理機制的套件（Flux、Redux、...），利用dispatch的機制換新頁面的語言呈現，在每個需要切換翻譯的組件裡，連結儲存裡的語言設定即可成就。',
        key:
          '為了保留語言設定，或者自動觸發切換語言之後的邏輯，大致上來說都會與儲存管理機制的套件做結合應用，但這邊還談及到一個不常使用的機制，倘若因應專案的開發需求，在UX的設計，想要讓網頁仍以重新載入網站的效果呈現語言切換時，可利用 React 中 key 的機制，在最頂層的節點加上 key 值，並綁定語言設定，故當語言切換後，更新儲存的語言設定，接著更新 key 值，最後因為 key 值不同，最頂層的組件被視為是新建立組件，使得整個網站已重新建立的方式，使用新指定語言的翻譯。但倘若實際的需求是要讓所有的檔案重新下載執行，仍須以重新載入的方式更新瀏覽器。',
      },
    },
    validator: {
      name: '驗證器',
