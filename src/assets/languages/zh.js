export default {
  ...
  package: {
    ...
    list: {
      ...
      description: {
        basic:
          '這個套件結合清單與選單的功能，利用展開收闔的功能，以及導頁的功能(React-router, history,...等)，便是後台網站常見的選單，配合我們設計的彈出框，還可成為下拉式選單。',
      },
      parameters: {
    ...
    table: {
      ...
      description: {
        basic:
          '表格的設計本身分成三種模式：列表（清冊表）、資訊（資料表）與資訊並列（比較表），使用視窗監聽，再配合客製化的外觀。即可完成漂亮的響應式的表顯示。',
      },
      parameters: {
        styleObj: '套件中任何具有 className 的節點，皆能使用 styleObj 定義 inline CSS。',
        slotObj:
    ...
    popover: {
      ...
      description: {
        basic:
          '彈出框預設有偵測視窗邊界的功能，可靈活的切換顯示位置與對齊方式，也具有強制鎖定顯示狀態的設計，另外如果結合彈出框與清單的套件，便可成為下拉式選單。',
      },
      parameters: {
        styleObj: '套件中任何具有 className 的節點，皆能使用 styleObj 定義 inline CSS。',
      },
      property: {
        showState: '控制彈出框顯示狀態。',
        showPosition: '控制彈出框顯示位置：top、right、bottom、left。',
        showAlign: '控制彈出框對齊位置：begin、center、end。',
        withArrow: '控制顯示為對話框。',
        autoDetect: '自動偵測式窗邊界功能，並自動切換彈出框顯示與對齊位置。預設為啟用。',
        trigger: '渲染彈出框觸發節點。',
        styleObj: '客製化外觀。',
        onToggle: '彈出框狀態切換事件函式。',
        onShow: '顯示彈出框事件函式。',
        onHide: '隱藏彈出框事件函式。',
        ref: '支援 React.ref。',
      },
    ...
    message: {
      ...
      property: {
        ...
        type: '訊息通知種類，套件本身已經附帶一些基本的樣式，當然您也可以自行客製，這個參數只是用來控制className，現成樣式：blue、grey、green、red、yello、light、dark。',
        context: '訊息通知的內容',
  ...
  application: {
    ...
    languages: {
      ...
      trigger: {
        ...
        key: '為了保留語言設定，或者自動觸發切換語言之後的邏輯，大致上來說都會與儲存管理機制的套件做結合應用，但這邊還談及到一個不常使用的機制，倘若因應專案的開發需求，在UX的設計，想要讓網頁仍以重新載入網站的效果呈現語言切換時，可利用 React 中 key 的機制，在最頂層的節點加上 key 值，並綁定語言設定，故當語言切換後，更新儲存的語言設定，接著更新 key 值，最後因為 key 值不同，最頂層的套件被視為是新建立套件，使得整個網站已重新建立的方式，使用新指定語言的翻譯。但倘若實際的需求是要讓所有的檔案重新下載執行，仍須以重新載入的方式更新瀏覽器。',
      },
    },
    validator: {
      name: '驗證器',
