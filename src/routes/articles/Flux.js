...
const introductionContent = (<span>{`In this article, we are going to talk about `}<a target='_blank' href='https://www.npmjs.com/package/flux'>{`Flux`}<FA icon={['fas', 'external-link-alt']} fixedWidth/></a>{`. If you need more clear example about learning Flux. The other article `}<a target='_blank' href='https://scotch.io/tutorials/getting-to-know-flux-the-react-js-architecture'>{`Getting To Know Flux, the React.js Architecture`}<FA icon={['fas', 'external-link-alt']} fixedWidth/></a>{` may be more suitable.`}</span>);

const introductionExampleTitle = `Example`;
const introductionExampleContent = (<span>{`The following shown is a user list. It is the example with our discussion to Flux. And here is source code of`} <a target='_blank' href='https://github.com/BlackToolBoxLaboratory/blacktbox-demo/tree/staging/version-1.0.0/src/example/articleFlux/example'>{`example`}<FA icon={['fas', 'external-link-alt']} fixedWidth/></a>{`.`}</span>);
const introductionExampleAlgorithmDescription = (<span>{`To implement the User List. Basically we need three factors: render(View), data(Store), access handler(Action). It is quite easy to implement it by coding them all together. However in systemic angle, if we want to separate them to keep system construction defined and used clear(to unify or generalize), we may need to use some framework to let system keep clear like Flux. And next, we will show it in Flux way. Comparing my `}<a target='_blank' href='https://blacktoolbox.cogman.org/react/v1/#/articles/flux'>{`Flux's example`}<FA icon={['fas', 'external-link-alt']} fixedWidth/></a>{`, I simplify the source code for more focus on the construction and add some notation, like this:`}</span>);
...
const advancedServerContent = (<span>{`Then, here is the User List work with server API. And also here is the source code of `}<a target='_blank' href='https://blacktoolbox.cogman.org/react/v1/#/articles/flux'>{`Flux's example with server`}<FA icon={['fas', 'external-link-alt']} fixedWidth/></a></span>);
